...
for rate in rate_list:
    ...
    for k in range(1, 10):
        ...
        #     rate = 2.0
        res_path = 'test_C_GD_vehicle_lane2_rate_' + str(rate) + '/'
        init_path = ''
        ...
        for info in user_config:
            info.update(config)
            user_list.append(MecTermGD(info, train_config, 'vehicle'))
        ...
        for i in range(MAX_EPISODE):
            ...
            for j in range(MAX_EPISODE_LEN):
                ...
                [cur_r, done, cur_p, cur_op, temp, cur_ts, cur_ps, cur_rs, cur_ds, cur_ch, cur_of, cur_pt, cur_pl,
                 cur_pv, sinr, overdata, cur_vs] = env.step_transmit()
                ...
                step_sumdata.append(cur_ps + cur_ts + cur_vs)
                ...
                step_power_vehicle.append(cur_pv)
                ...
                if done:
                    ...
                    print('%d:r:%s,p:%s,[%s,%s,%s]dbuf:%s,noise:%s' % (
                    i, cur_r_ep / MAX_EPISODE_LEN, cur_p_ep / MAX_EPISODE_LEN, cur_power_offload / MAX_EPISODE_LEN,
                    cur_power_local / MAX_EPISODE_LEN, cur_power_vehicle / MAX_EPISODE_LEN, cur_ds_ep / MAX_EPISODE_LEN,
                    cur_noise_ep / MAX_EPISODE_LEN))

            # plt.ioff()
            # plt.show
            step_result = step_result_path + 'step_result_' + time.strftime("%b_%d_%Y_%H_%M_%S",
