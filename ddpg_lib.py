...
# ===========================
#   Actor and Critic DNNs         DNN深度神经网络 CNN卷积神经网络
# ===========================

class ActorNetwork(object):
    """
    Input to the network is the state, output is the action
    under a deterministic policy.     决定性策略
    ...
    def __init__(self, sess, state_dim, action_dim, action_bound, learning_rate, tau, batch_size, user_id=''):
        ...
        self.action_bound = action_bound  #动作的幅度范围
        self.learning_rate = learning_rate #学习率
        self.tau = tau  #对应一个系数
        self.batch_size = batch_size
        self.user_id = user_id

        start_idx = len(tf.global_variables())  # 全局变量的列表长度
        
        start_idx_train = len(tf.trainable_variables())  # 训练变量的列表长度
        # Actor Network
        self.inputs, self.out, self.scaled_out = self.create_actor_network()  # 函数在后面，创建神经网络
        ...
        # Target Network
        self.target_inputs, self.target_out, self.target_scaled_out = self.create_actor_network() # 函数在后面，创建神经网络

        self.target_network_params = tf.trainable_variables()[end_idx_train:]
        
        self.params = tf.global_variables()[start_idx:]

        # Op for periodically updating target network with online network  #  更新网络
        # weights
        self.update_target_network_params = \
            [self.target_network_params[i].assign(tf.multiply(self.network_params[i], self.tau) +
                                                  tf.multiply(self.target_network_params[i], 1. - self.tau))
                for i in range(len(self.target_network_params))]    #更新target里面的i个参数

        # This gradient will be provided by the critic network
        self.action_gradient = tf.placeholder(tf.float32, [None, self.a_dim])   #定义一个维度
        #tf.placeholder()，我们每次可以将 一个minibatch传入到x = tf.placeholder(tf.float32,[None,32])上，下一次传入的x都替换掉上一次传入的x，这样就对于所有传入的minibatch x就只会产生一个op，不会产生其他多余的op，进而减少了graph的开销。
        ...
        # Combine the gradients here
        grads = tf.gradients(
            self.scaled_out, self.network_params, -self.action_gradient)   #求梯度, 第三个参数是被相乘的。链式法则  用到了helper里面的改进
        # the gradient shoudl not be None     scaled_out是策略   看一下莫凡python
        self.unnormalized_actor_gradients = [grad if grad is not None else tf.zeros_like(var) 
                                             for var, grad in zip(self.network_params, grads)]  #python语法
        self.actor_gradients = list(map(lambda x: tf.div(x, self.batch_size), self.unnormalized_actor_gradients))
        #梯度
        # Optimization Op    优化操作
   ...
   #梯度上升  adm方法  更新参数
    ...
    def create_actor_network(self):  #创建网络
...
class CriticNetwork(object):
    ...
    def __init__(self, sess, state_dim, action_dim, learning_rate, tau, gamma, num_actor_vars):
        ...
        # weights with regularization   更新参数  （27）（28）
        self.update_target_network_params = \
            [self.target_network_params[i].assign(tf.multiply(self.network_params[i], self.tau) \
            + tf.multiply(self.target_network_params[i], 1. - self.tau))
...
class DeepQNetwork(object):   #DQN网络
    """
...
class OrnsteinUhlenbeckActionNoise:  #探索噪声，DDPG采用OU过程
