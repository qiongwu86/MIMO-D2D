...
for k in range(11,20):
    ...
    NUM_T = 1
    NUM_R = 4
    SIGMA2 = 1e-9  #信道噪声的参数方差

    t_factor = 0.9
    noise_sigma = 0.02  #探索噪声的参数

    config = {'state_dim':3, 'action_dim':3};
  ...
  #random_seed是返回一个固定的时间计数值      gamma为折扣因子     actor_lr为学习率
    IS_TRAIN = False

    res_path = 'train_C_ddpg_sigma0_02_rate3_lane2/'
    model_fold = 'model_C_ddpg_sigma0_02_rate3_lane2/'
    model_path = 'model_C_ddpg_sigma0_02_rate3_lane2/my_train_model_rate_3_' + str(k) + '-2000'

    if not os.path.exists(res_path):
        os.mkdir(res_path)    #路径不存在则创建目录
    if not os.path.exists(model_fold):
        os.mkdir(model_fold)   #同上
    ...
    user_list = [];
    for info in user_config:           #————————训练模型放入user_list，使user_list具有类的功能（MecTermRL），获取类的地址
        info.update(config)
        ...
        user_list.append(MecTermRL(sess, info, train_config))  #mec-314    获取用户类的地址
        print('Initialization OK!----> user ' + info['id'])

    # 2. create the simulation env
    env = MecSvrEnv(user_list, Train_vehicle_ID, SIGMA2, MAX_EPISODE_LEN)  #mec_envP427
    ...
    for i in range(MAX_EPISODE):
        ...
        for j in range(MAX_EPISODE_LEN):
            ...
            [cur_r, done, cur_p, cur_op, temp, cur_ts, cur_ps, cur_rs, cur_ds, cur_ch, cur_of, cur_pt, cur_pl,cur_pv, sinr, overdata,cur_vs] = env.step_transmit()
                   #训练 下面是画图
            cur_r_ep += cur_r
            cur_p_ep += cur_p
            cur_op_ep += cur_op
            ...
            if done:
                ...
                res_p_vehicle.append(cur_power_vehicle/MAX_EPISODE_LEN)
                res_d.append(cur_ds)
                ...
                try:
                    ...
                    ax.lines.remove(line_vehiclepower[0])
                    ...
                    ax4.lines.remove(line_vehiclebits[0])
                ...
                line_vehiclepower = ax.plot(range(0,MAX_EPISODE_LEN),step_pv,'#800080',label='vehiclepower:W',lw=1) #颜色为偏紫色
                ...
                line_vehiclebits = ax4.plot(range(0,MAX_EPISODE_LEN),step_vehicle_bit,'b-',color='#800080',label='vehiclebits:kbit',lw=1) #颜色为偏紫色
                ...
                print('%d:r:%s,p:%s,[%s,%s,%s]dbuf:%s,noise:%s'%(i,cur_r_ep/MAX_EPISODE_LEN,cur_p_ep/MAX_EPISODE_LEN,cur_power_offload/MAX_EPISODE_LEN,cur_power_local/MAX_EPISODE_LEN,cur_power_vehicle/MAX_EPISODE_LEN,cur_ds_ep/MAX_EPISODE_LEN,cur_noise_ep/MAX_EPISODE_LEN))
    ...
    np.savez(name, res_r, res_p, res_b, res_o, res_d, res_p_local, res_p_offload,res_p_vehicle)
    
    tflearn.config.is_training(is_training=False, session=sess)  #训练结束
